...
import { getUserFileCategories, TuningCategory } from '../tuning-options'; // Изменили импорт

// Генерация моковых данных файлов пользователя
const generateUserFiles = (): UserFile[] => {
  const files: UserFile[] = [];
  const categories = getUserFileCategories(); // Используем переименованную функцию
      ...
      generation.engines.forEach(engine => {
        categories.forEach((category: string, index: number) => {
          ...
          const file: UserFile = {
            ...
            fileSize: Math.round((Math.random() * 20 + 5) * 100) / 100, // 5-25 MB
            uploadDate: new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000), // последние 30 дней
            status,
